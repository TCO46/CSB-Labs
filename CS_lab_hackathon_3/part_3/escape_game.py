...
#     player_x_list = list(list(zip(*numpy.where(array_2d == "P")))[0])[1]
#     player_y_list = list(list(zip(*numpy.where(array_2d == "P")))[0])[0]
#     if direction == "w":
#         player_y_list += 1
#     elif direction == "d":
#         player_x_list += 1
...
#     return player_y_list, player_x_list
...
# player_x = list(zip(*numpy.where(array_2d == "P")))[0][0]
# player_y = list(zip(*numpy.where(array_2d == "P")))[0][1]

# key_x = list(zip(*numpy.where(array_2d == "K")))[0][0]
# key_y = list(zip(*numpy.where(array_2d == "K")))[0][1]

# door_x = list(zip(*numpy.where(array_2d == "D")))[0][0]
# door_y = list(zip(*numpy.where(array_2d == "D")))[0][1]

# print(player_x, player_y, key_x, key_y, door_x, door_y)
...
def generate_map(rows, cols):
    # initialize empty map as a list of list
...
def move(map, rows, cols, ch, r_P, c_P):
    ...
    # set the value of old Player's position to empty
    map[r_P][c_P] = '-'
    # set the value of new Player's position to 'P'
